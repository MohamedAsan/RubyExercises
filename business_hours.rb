...
class BusinessHours
	...
	def get_opening_time(given_day)
		return @opening_times[given_day]
	end

	def get_closing_time(given_day)
	...
	def calculate_deadline(time_interval, given_starting_time)
		...
		opens_at = Time.parse(get_opening_time(starting_day))
		closes_at = Time.parse(get_closing_time(starting_day))
